...
public class DiscordRPCMod extends Module {
	...
	public void onEnable() {
		...
		tick = 0;
		DiscordRPCManager.start(silent ? "794178232084856833" : "794178232084856833");
	...
	public void onTick(EventTick event) {
		...
		if (tick % 40 == 0) {
			...
			case 0:
				if (mc.getCurrentServerEntry() != null && !mc.getCurrentServerEntry().address.contains(".*\\d.*")) text1 = "Playing " + mc.getCurrentServerEntry().address;
				else text1 = "Playing Singleplayer or homehosted server";

				break;
			case 1:
				if (mc.getCurrentServerEntry() != null && !mc.getCurrentServerEntry().address.contains(".*\\d.*")) text1 = mc.getCurrentServerEntry().address;
			...
			DiscordRPC.discordUpdatePresence(
					...
					.setBigImage("machacklogo", silent ? "Minecraft " + SharedConstants.getGameVersion().getName() : "ApeHack " + MacHack.VERSION)
